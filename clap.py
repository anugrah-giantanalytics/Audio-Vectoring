...
# Initialize Qdrant client
    ...
    def test_search_by_text(self, query_text: str):
        ...
        try:
            ...
            if not results:
                print("No results found via Qdrant. Trying fallback search...")
                results = self.fallback_search_by_text(query_text, k=2)
                
            print(f"\nResults:")
            for i, (doc, score) in enumerate(results):
                print(f"  Result {i+1}: {doc.page_content[:100]}{'...' if len(doc.page_content) > 100 else ''}")
                if 'start_time' in doc.metadata and 'end_time' in doc.metadata:
                    print(f"    Time range: {doc.metadata['start_time']:.2f}s - {doc.metadata['end_time']:.2f}s")
    ...
    def fallback_search_by_text(self, query_text: str, k: int = 3):
        """Fallback method for text search that uses saved embeddings"""
        try:
            import os
            import glob
            import pickle
            ...
            print("Performing fallback text-based search on saved embeddings...")
            
            # Find all audio embedding files
            embedding_files = glob.glob("chunks/clap/**/chunk_*_text_embedding.pkl", recursive=True)
            ...
            # Encode query
            text_inputs = text_processor(text=query_text, return_tensors="pt", padding=True)
            if torch.cuda.is_available():
                text_inputs = {key: val.cuda() for key, val in text_inputs.items()}
                text_model = text_model.cuda()
            
            with torch.no_grad():
                query_embedding = text_model(**text_inputs).text_embeds
                query_embedding = query_embedding / query_embedding.norm(dim=-1, keepdim=True)
                query_embedding = query_embedding.cpu().numpy()[0]
            
            # Load saved embeddings and compare
            results = []
            for file_path in embedding_files:
                try:
                    ...
                    audio_file = file_path.replace('_text_embedding.pkl', '.wav')
                    metadata_file = file_path.replace('_text_embedding.pkl', '_metadata.json')
                    metadata = {}
                    ...
                    # Create document
                    content = f"Audio chunk {metadata.get('chunk_id', 'unknown')}"
                    if 'start_time' in metadata and 'end_time' in metadata:
                        content += f" (Time: {metadata['start_time']:.2f}s - {metadata['end_time']:.2f}s)"
                    
                    results.append((Document(page_content=content, metadata=metadata), float(similarity)))
